...
2. Particle downsampling: Find maximum downsample within --tolerance of the baseline
3. Galaxy downsampling: Find maximum downsample within --tolerance of the baseline
...
Usage:
    python benchmark_dsigma_convergence.py --halo_path halos.parquet \
        --particle_path particles.parquet

Its baseline cache is its own (ELG_GHOD); it is not the one written by
numerical_dsigma_example.py, which uses a different HOD.
"""
...
parser.add_argument("--halo_path", required=True, help="halo catalogue (parquet)")
parser.add_argument("--particle_path", required=True, help="particle catalogue (parquet)")
parser.add_argument("--tolerance", type=float, default=0.01,
                    help="max fractional deviation from the baseline (default 1%%)")
...
TOLERANCE = args.tolerance   # max fractional deviation from the baseline

BASELINE_CACHE = "benchmark_baseline_cache.npz"
